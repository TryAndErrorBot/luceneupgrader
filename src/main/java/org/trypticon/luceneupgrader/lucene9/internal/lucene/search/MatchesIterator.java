 ...
 *
 * <p>To iterate over the matches, call {@link #next()} until it returns {@code false}, retrieving
 ...
 *
 * <p>Matches from some queries may span multiple positions. You can retrieve the positions of
 * individual matching terms on the current match by calling {@link #getSubMatches()}.
 *
 * <p>Matches are ordered by start position, and then by end position. Match intervals may overlap.
 *
...
public interface MatchesIterator {
  ...
  /**
   * Returns a MatchesIterator that iterates over the positions and offsets of individual terms
   * within the current match
   *
   * <p>Returns {@code null} if there are no submatches (ie the current iterator is at the leaf
   * level)
   *
   * <p>Should only be called after {@link #next()} has returned {@code true}
   */
   ...
   *
   * <p>If this {@link MatchesIterator} has been returned from a {@link #getSubMatches()} call, then
   * returns a {@link TermQuery} equivalent to the current match
   *
   * <p>Should only be called after {@link #next()} has returned {@code true}
   */
  Query getQuery();
