 ...
 *
 * <p>See the {@link org.trypticon.luceneupgrader.lucene9.internal.lucene.util.fst package documentation} for some simple examples.
 *
 * @lucene.experimental
 */
public final class FST<T> implements Accountable {
  ...
  public Arc<T> findTargetArc(int labelToMatch, Arc<T> follow, Arc<T> arc, BytesReader in)
    ...
    if (flags == ARCS_FOR_DIRECT_ADDRESSING) {
      ...
      } else if (!Arc.BitTable.isBitSet(arcIndex, arc, in)) {
