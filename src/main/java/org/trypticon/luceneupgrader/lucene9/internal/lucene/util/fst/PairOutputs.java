...
public class PairOutputs<A, B> extends Outputs<PairOutputs.Pair<A, B>> {
  ...
  public Pair<A, B> common(Pair<A, B> pair1, Pair<A, B> pair2) {
    ...
    assert valid(pair2);
    return newPair(
        outputs1.common(pair1.output1, pair2.output1),
  ...
  public Pair<A, B> add(Pair<A, B> prefix, Pair<A, B> output) {
    ...
    assert valid(output);
    return newPair(
        outputs1.add(prefix.output1, output.output1), outputs2.add(prefix.output2, output.output2));
  }
  ...
  public String outputToString(Pair<A, B> output) {
    assert valid(output);
    return "<pair:"
        + outputs1.outputToString(output.output1)
        + ","
